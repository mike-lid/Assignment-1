...
**VERSION 05**
```
In the fifth version, we formatted a gray background to help the map stand out on the page, adjusting text size and label fonts so that the focus of the image remains on the map. We added a scale bar and a north arrow for orientation, allowing map users to quickly understand the density and size of the parking spots in relation to the parks. In this version, we chose to represent parking with warmer colors, so that there is a strong visual contrast between the open spaces (green) and those spaces paved and occupied for parking (warm color tones).
